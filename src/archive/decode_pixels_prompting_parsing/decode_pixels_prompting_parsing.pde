...
//  port = new Serial(this, "COM5", 115200);
  port = new Serial(this, "COM5", 230400);
//  port = new Serial(this, "COM5", 500000);  // didn't work at all
//  port = new Serial(this, "COM5", 250000);  // didn't work at all
...
// Flag to toggle start transmission
boolean startTransmission = true;
...
int sensora, sensorb, sensorc, sensord, sensore;
...
void serialEvent(Serial port) {
  ...
  // Detect the packet type
   if(inString.indexOf("$ITRDY") == 0) {            //Ready packet with sensor information
     
    // Arduino ready to send
    println("Ready");
    ...
    if(list.length != 7) {
      ...
      sensora = Integer.parseInt(list[1]);
      sensorb = Integer.parseInt(list[2]);
      sensorc = Integer.parseInt(list[3]);
      sensord = Integer.parseInt(list[4]);
      
      // Print the info to console
      println("sensor a: " + sensora);
      println("sensor b: " + sensorb);
      println("sensor c: " + sensorc);
      println("sensor d: " + sensord);
      println("sensor e: " + list[5]);
      
      }

    } else if(inString.indexOf("$ITHDR") == 0) {    //Image header packet with image information
  ...
  } else if(inString.indexOf("$ITDAT") == 0) {      //Image data packet
